 ...
 * <LI>An abstraction of the Dockable notion, with factories to allow different implementations or customization of the
 * Dockable containers.
 ...
 *          <li>update 2005/11/08 Lilian Chamontin : added support for global width/height drop (in split method)</li>
 *          <li>update 2005/11/14 Lilian Chamontin : reworked setFloating methods.</li>
 *          <li>update 2005/12/08 Lilian Chamontin : fixed a bug related to multiple desktop usage (when moving a
 *          window</li> after the desktop has been removed from hierarchy, the listeners were still invoked and caused a
 *          NPE in moveFloatingWindows).</li>
 *          <li>update 2006/12/01 Lilian Chamontin : fixed a NPE when closing a window with some floating children
 *          remaining</li>
...
public class DockingDesktop extends JLayeredPane {
    ...
    /**
     * a flag set when adding a maximized component: true, this one is heavyweight Only used with heavyweight support
     * AND singleHeavyWeightComponent
     */
    ...
    public DockingDesktop(String desktopName, DockingContext context) {
        ...
        borderPanes = new AutoHideButtonPanel[] { topBorderPane, leftBorderPane, bottomBorderPane, rightBorderPane };
        ...
        expandPanel
            .addPropertyChangeListener(AutoHideExpandPanel.PROPERTY_EXPANDED,
         ...
         * focusHandler); //2006/08/21 : back to life (was in comments in 2.1.0)
         */

        installKeyboardBindings();

        // optional window following
        final boolean floatingContainerFollowParentWindow =
            UIManager.getBoolean("FloatingContainer.followParentWindow");
        addAncestorListener(new AncestorListener() {

            @Override
            public void ancestorAdded(AncestorEvent event) {
                KeyboardFocusManager
                    .getCurrentKeyboardFocusManager().addPropertyChangeListener("focusOwner", focusHandler); // 2006/08/21
                // :
                // back
                // to
                // life
            ...
            public void ancestorRemoved(AncestorEvent event) { // 2005/12/08
                KeyboardFocusManager
                    .getCurrentKeyboardFocusManager().removePropertyChangeListener("focusOwner", focusHandler); // 2006/12/19
    ...
    /**
     * Registers listeners for notable keyboard events (CLOSE, DOCK/HIDE, MAXIMIZE/RESTORE, float/attach). KeyStroke
     * bindings can be defined and overriden this UIManager properties (see DockingUISettings for details).
     *
     */
     ...
     * <p>
     * Transparent contents allow adding background images/information when no dockable is visible (everything closed or
     * autohidden).
     * <p>
    ...
    /**
     * Returns the docking panel used by this desktop. Usage of this method should be limited to VLDocking extensions
     * (simple users shouldn't rely on the underlying DockingPanel existence)
     */
     ...
     *
     * <P>
     * Registration is automatic for shown dockables : methods setCentralDockable(), split(), createTab(), hide()...
     * leading to have the dockable visible call registerDockable() )
     * <P>
     * for not shown dockable (in order to read a configuration from an XML stream, or to list the dockable in
     * DockingSelectorDialog ), this method must be called manually.
     * <p>
    ...
    /**
     * Returns a String containing the version of the docking framework in the format M.m.r where M is the major, m the
     * minor and r the release.
     *
     * @since 2.0
     */
     ...
     * @param dockable
     *            a dockable to add at the same position instead of <code>base</code>. if base is not already child of a
     *            tabbedpane, a new tabbedpane will be created and inserted at base's location.
     * @param order
     ...
     *
     * @param base
     *            the existing dockable, either displayed in a DockableContainer or in a TabbedDockableContainer.
     *            <P>
    ...
    /**
     * private implementation wich allows for triggering state change event or not (depending on the caller).
     */
    private void createTab(Dockable base, Dockable dockable, int order, boolean select, boolean triggerEvents) {
        /*
         * createTab() is called by : - DockView/DetachedDockView during DnD - DockTabbedPane during DnD - show() which
         * is called by - setFloating(false) - setAutoHide(false)
         */
        ...
        DockingActionEvent dae =
            new DockingActionCreateTabEvent(this, dockable, currentLocation, newState.getLocation(), base, order);
        ...
        if (baseTab != null) {
            ...
            DockingUtilities
                .replaceChild(((Component) baseOldContainer).getParent(), (Component) baseOldContainer,
                    (Component) baseTab);
        }
        if (select) {
     ...
     * <p>
     * If the base is contained in a non-splitable container (like a tab of DockTabbedPane) then, a splittable ancestor
     * will be searched (until the root desktop pane is reached) to apply splitting.
     *
     * @param base
     ...
     * @param proportion
     *            the proportion of the initial dockable space taken by the new dockable a negative proportion, like -1,
     *            will be ignored (and split will be based on component preferred sizes and weights). This parameter is
     *            an alternative to DockingDesktop.setDockableHeight() and setDockableWidth() methods
    ...
    public void split(Dockable base, Dockable newDockable, DockingConstants.Split position, double proportion) {
        /*
         * split() is used internally by the framework in the following cases : - from a move() API invocation - from a
         * DnD on DockTabbedPane (overriding BorderSplitter) but only when the dockable is not floating
         *
        ...
        DockingActionEvent dae =
            new DockingActionSplitDockableEvent(this, newDockable, currentLocation, newState.getLocation(), base,
                position, 0.5f);
     ...
     * The base dockable is the reference, the second newDockable will be added according to the position parameter.
     * <p>
     * If base is contained in a non splitable container (like a tab of DockTabbedPane) then, a splitable ancestor will
     * be searched (until the root desktop pane is reached) to apply splitting.
     *
     * @param base
     *            an already docked Dockable
     * @param newDockable
     ...
     * <p>
     * <b>Note : </b> This method is for DockableContainers. API users should use the
     * {@link #split(Dockable,Dockable,DockingConstants.Split) split} method.
     *
     * @param base
     *            the reference component (a dockablecontainer or a split container)
     * @param dockable
     *            the dockable to add
     * @param position
    ...
    public void splitComponent(Component base, Dockable dockable, DockingConstants.Split position) {
        /*
         * splitComponent() is called from SplitContainer and BorderSplitter BorderSplitter is used by : - DockView (not
         * floating, as the DetachedDockView child overrides the scanDrop method and doesn't use the BorderSplitter -
         * DockTabbedPane (only when not floating) SplitContainer is used for DnD operations that can occur from
         * Hide/Docked/Floating elements
         *
         * => The future state will be DOCKED, but we cannot guess the current state (HIDE/FLOAT/DOCKED)=> as of 2.1 :
         * the future state can be DOCKED or FLOATING (with the help of compound dockable)
         */
        ...
        /*
         * if (!dockingPanel.isAncestorOf(base)){ futureState = DockableState.FLOATING; }
         */
        ...
        DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
        DockingActionEvent dae =
            new DockingActionSplitComponentEvent(this, dockable, currentLocation, newState.getLocation(), base,
                position, 0.5f);
    ...
    /**
     * Replaces the base component by a split pane oriented according to position, and put the dockablesContainer at
     * position.
     * <p>
     * <b>Note : </b> This method is for DockableContainers. API users should use the
     * {@link #split(Dockable,Dockable,DockingConstants.Split) split} method.
     ...
     * <p>
     * The dockable is inserted in the docking desktop where it was before, if possible. if not, an approximated place
     * will be looked for.
     */
    ...
    public void remove(Dockable dockable) {
        ...
        if (dockLocation == DockableState.Location.FLOATING) {
            FloatingDockableContainer fdc =
                (FloatingDockableContainer) SwingUtilities.getWindowAncestor(dockable.getComponent());
             ...
             *
             * Window w = SwingUtilities.getWindowAncestor(dockable.getComponent()); w.dispose(); } else { // well, it's
             * not shown, and not hidden... // nop }
             */
        }
    }
    ...
    public void dropRemove(DockableDragSource dragSource) {
        ...
        if (state != null) {
            ...
            DockableState updatedState =
                new DockableState(state, new RelativeDockablePosition(getRelativeAncestorContainer(d), d));
    ...
    private void remove(TabbedDockableContainer tdc) {
        ...
        if (dockLocation == DockableState.Location.FLOATING) {
            FloatingDockableContainer fdc =
                (FloatingDockableContainer) SwingUtilities.getWindowAncestor((Container) tdc);
            DockingUtilities.dispose(fdc);
            // ((JDialog)dlg).dispose();
        }
    ...
    public void maximize(Dockable dockable) {
        ...
        DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
        DockingActionEvent dae =
            new DockingActionSimpleStateChangeEvent(this, dockable, currentLocation, DockableState.Location.MAXIMIZED);
        ...
        SingleDockableContainer maxDockableContainer =
            DockableContainerFactory
                .getFactory().createDockableContainer(dockable, DockableContainerFactory.ParentType.PARENT_DESKTOP);
        ...
        if (DockingPreferences.isLightWeightUsageEnabled()) {
            ...
            if (DockingPreferences.isSingleHeavyWeightComponent()) {
                ...
                if (DockingUtilities.isHeavyWeightComponent(comp)) {
                    ...
                    maximizedComponent
                        .setBounds(i.left, i.top, getWidth() - i.left - i.right, getHeight() - i.top - i.bottom);
                    add(maximizedComponent, JLayeredPane.PALETTE_LAYER);
    ...
    public void restore(final Dockable dockable) {
        ...
        DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
        DockingActionEvent dae =
            new DockingActionSimpleStateChangeEvent(this, dockable, currentLocation, DockableState.Location.DOCKED);
    ...
    /**
     * Used only with DOCKED dockables : returns the nearest ancestor container (can be a CompoundDockingPanel if
     * nested, or the DockingPanel).
     */
    ...
    public void setFloating(final Dockable dockable, boolean floating, Point screenPosition) {
        ...
        if (floating) {
            ...
            if (location == DockableState.Location.FLOATING) {
                ...
                DockingActionEvent dae =
                    new DockingActionSimpleStateChangeEvent(this, dockable, currentLocation,
            ...
            DockableState newState =
                new DockableState(this, dockable, previousState.getLocation(), previousState.getPosition());
            DockableStateWillChangeEvent event = new DockableStateWillChangeEvent(currentState, newState);
            DockingActionEvent dae;
            if (currentState != null && currentState.isFloating()) {
                if (previousState.isDocked()) {
                    ...
                    dae =
                        RelativeDockingUtilities
                            .getInsertionDockingAction(relativeAncestor, dockable, currentState, newState);
                }
                else if (previousState.isHidden()) { // it was hidden
                    dae =
                        new DockingActionSimpleStateChangeEvent(this, dockable, DockableState.Location.FLOATING,
    ...
    private boolean checkDockableStateWillChangeToFloating(TabbedDockableContainer tdc) {
        for (int i = 0; i < tdc.getTabCount(); i++) {
            ...
            DockableStateWillChangeEvent event = new DockableStateWillChangeEvent(currentState, newState);
            DockingActionEvent dae =
                new DockingActionSimpleStateChangeEvent(this, d, currentLocation, newState.getLocation());
            if (!isDockingActionAccepted(dae, event)) {
                return false;
            }
        }
        ...
        if (dockable instanceof CompoundDockable) {
            ...
            for (int i = 0, childrenSize = children.size(); i < childrenSize; i++) {
                ...
                previousFloatingDockableStates
                    .put(d, new DockableState(this, d, state.getLocation(), state.getPosition()));
            }
        }
    }
     ...
     * <p>
     * This method is protected to give access to implementors wanting to customize its look and feel (for example :
     * removing the title bar by setting it to "undecorated").
     * <p>
     * The old API has been removed (it returned a JDialog) as the DockingDesktop can only work with
     * FloatingDockableContainers.
     */
    protected FloatingDockableContainer createFloatingDockableContainer(final Dockable dockable) {
        Window ownerWindow = SwingUtilities.getWindowAncestor(this);
        FloatingDockableContainer fdc =
            DockableContainerFactory.getFactory().createFloatingDockableContainer(ownerWindow);
    ...
    protected void removeContainer(SingleDockableContainer dc) {
        ...
        /*
         * to be improved : this method supposes a direct ancestor link between DockableContainer and
         * TabbedDockableContainer
         */
                ...
                if (parent instanceof SplitContainer) {
                    ...
                    if (tparent.getTabCount() == 1) { // no more use for tabs
                        ...
                        if (floating) {
                            lastContainer =
                                DockableContainerFactory
                                    .getFactory().createDockableContainer(last,
                        ...
                        DockingUtilities
                            .replaceChild(((Component) tparent).getParent(), (Component) tparent,
     ...
     * <P>
     * the dockable remains registered and can later be shown again at the same location, using addDockable(Dockable,
     * RelativeDockablePosition).
     * <p>
     * To have access to the RelativeDockablePosition at closing time, one has to install a DockableEventListener like
     * this :
     *
     * <pre>
    ...
    public void close(Dockable dockable) {
        ...
        if (dockingPanel.isAncestorOf(dockable.getComponent())) {
            ...
            if (isDockingActionAccepted(dae, dswe)) {
                ...
                context
                    .setDockableState(dockable,
                        new DockableState(this, dockable, DockableState.Location.CLOSED, currentState.getPosition()));
                dockable.getDockKey().setLocation(DockableState.Location.CLOSED);
                fireDockingAction(dae, new DockableStateChangeEvent(currentState, newState));
            }
        }
    ...
    /**
     * Iterates through this compound children and properly close them (without removing them from their parent
     * container)
     */
     ...
     * <p>
     * Note : is a dockable is not allowed to close (key.isCloseEnabled() returns false) then it won't be closed by this
     * method.
     * <p>
    ...
    public void setAutoHide(Dockable dockable, boolean hide) {
        /*
         * setAutoHide is called by user applications (or addHidenDockable() ) and from inside the framework : - Tab
         * actions (only when in the DOCKED state) - DockView (listening to DockViewTitleBar property change - key
         * events from the desktop - the AutoHideExpandPanel listening to DockViewTitleBar changes
         *
         * this method is not used when hiding from FLOATING state, the "floatingToHide" method is used istead to avoid
         * triggering unused events
         */
        DockKey k = dockable.getDockKey();
        ...
        if (hide) {
            ...
            RelativeDockablePosition position =
                new RelativeDockablePosition(getRelativeAncestorContainer(dockable), dockable);
            DockableState newState = new DockableState(this, dockable, DockableState.Location.HIDDEN, position);
            DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
            DockingActionEvent dae =
                new DockingActionSimpleStateChangeEvent(this, dockable, currentLocation, DockableState.Location.HIDDEN);
            ...
            DockingActionEvent dae =
                RelativeDockingUtilities.getInsertionDockingAction(relativeContainer, dockable, currentState, newState);
    ...
    /**
     * This method is used by DockableContainers in order to register their DockableDragSource(s) to the
     * OldDragControler.
     * <p>
     * Adds a MouseListener and a MouseMotionListener to the Component-DockableDragSource <code>source</code>. These
     * listeners are used to perform start-dragging-docking actions over the source component
     */
    ...
    /**
     * This method is used by DockableContainers in order to unregister their DockableDragSource(s) to the
     * OldDragControler.
     * <p>
     ...
     * <p>
     * As of version 2.1, this method delegates the work to DockingContext.writeXML, which will export every dekstop
     * sharing the same context (and not only this one)
     *
    ...
    void writeDesktopNode(PrintWriter out) throws IOException {
        ...
        out
            .println("<DockingPanel x=\"" + x + "\" y=\"" + y + "\" width=\"" + width + "\" height=\"" + height + "\""
                + ">");
        if (dockingPanel.getComponentCount() > 0) {
    ...
    private void xmlWriteFloating(PrintWriter out) throws IOException {
        ...
        Point windowLocation;
        /*
         * if (desktopWindow != null) { windowLocation = desktopWindow.getLocation(); } else { windowLocation = new
         * Point(); // fail safe }
         */
        ...
        for (int i = 0; i < floatingDockables.size(); i++) {
            ...
            out
                .println("<Floating x=\"" + (r.x) + "\" y=\"" + (r.y) + "\" width=\"" + r.width + "\" height=\""
                    + r.height + "\">");
    ...
    private void xmlWriteDockableWithRelativePosition(Dockable dockable, PrintWriter out) throws IOException {
        ...
        out.println("<Key dockName=\"" + key.getKey() + "\"/>");
        out
            .println("<RelativePosition x=\"" + position.getX() + "\" y=\"" + position.getY() + "\" w=\""
                + position.getWidth() + "\" h=\"" + position.getHeight() + "\" />");
        if (isCompound) {
            CompoundDockable d = (CompoundDockable) dockable;
            CompoundDockingPanel cdp = (CompoundDockingPanel) d.getComponent(); // no
            // choice
    ...
    /**
     * TabGroups are the memory of tabs (even for hidden or floating dockables) used to put them back on the right tab
     * when docked again
     */
     ...
     * <p>
     * When the method returns, the desktop is totally reconfigured with posiibly different dockable at different
     * positions.
     * <p>
     * <b>Note : </b> The <code>DockKey</code>s of the stream must be registered with the
     * {@link #registerDockable(Dockable) registerDockable} method, prior readXML. <br>
     * This is the case if the desktop is already open and dockables laid out, but might not be the case if this method
     * is used at application startup to populate an empty desktop.
     *
     * <p>
    ...
    private void xmlBuildFloatingNode(NodeList dockables, Rectangle bounds) throws SAXException {
        ...
        for (int i = 0; i < dockables.getLength(); i++) {
            ...
            if (fdc == null) {
                ...
                if (tdc != null) {
                    if (tdc.getTabCount() == 0) {
                        ...
                        DockingUtilities
                            .replaceChild(((Component) base).getParent(), (Component) base, (Component) tdc);
            ...
            context
                .setDockableState(dockable,
                    new DockableState(this, dockable, DockableState.Location.FLOATING, position));
            dockable.getDockKey().setLocation(DockableState.Location.FLOATING);
    ...
    private void xmlBuildCompoundDockable(
        CompoundDockable dockable, Element compoundElt, DockableState.Location dockableLocation) throws SAXException {
    ...
    private TabbedDockableContainer xmlBuildTabbedDockableContainer(
        Element elt, DockableState.Location dockableLocation) throws SAXException {
     ...
     * <ul>
     * <li>the desktop is empty (when not empty, you can use split() or createTab() to add a dockable relatively to
     * another)
     * <li>the dockable is currently closed, and it must be shown again
     * <li>note that if the desktop is not empty, the dockable will be added in the bottom of the desktop (equivalent to
     * addDockable(dockable 0,0.8,1,0.2)).
     * </ul>
     */
     ...
     * <p>
     * This method is used to reposition a closed dockable at its previous location on the desktop. As it relies on the
     * Component.findComponentAt(Point) method , the desktop must already be visible.
     *
     * <p>
     * The preferred way to obtain a particular visual docking configuration is to use a combination of add(), split()
     * and createTab() methods, as these methods do not rely on an interpretation (and approximation) of constraints.
     *
     * <p>
     * Precision of constraints : as the docking management is based on a mix of horizontal and vertical splitting
     * zones, it is not always possible to respect the constraints given.
     * <p>
     ...
     * <li>sub-split the splitter (horizontally or vertically) zone and position the Dockable in the most appropriate
     ...
     *
     * @throws IllegalArgumentException
     *             if the dockable already belongs to the desktop containment hierarchy.
     */
    public void addDockable(Dockable dockable, RelativeDockablePosition position) {
        ...
        DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
        DockingActionEvent dae =
            new DockingActionAddDockableEvent(this, dockable, currentLocation, newState.getLocation(), dockingPanel);
        if (isDockingActionAccepted(dae, dswe)) {
     ...
     * <p>
     * Useful for example to reserve some space on a desktop by using a "placeholder' when other dockables aren't
     * visible (Multiple Tabbed Document Interface with always a dockable visible even when no document is loaded).
     * <p>
     * Implementation note : this method assumes only the "base" dockable is visible (the replacer must be in the CLOSED
     * state). It also assumes the base dockable isn't a compound dockable. These limitations will be removed in a later
     * release.
     *
     * <p>
     * There is currently no DnD gesture associated to this action (although the "HotSwap" gesture would be a good
     * candidate). So this method currently doesn't trigger any event (state change, action), yet it could change later.
     *
     * @since 2.1
     * @throws IllegalArgumentException
        ...
        if (tdcBase != null) {
            ...
            if (sdcBaseParent instanceof SplitContainer) {
                ...
                SingleDockableContainer sdc =
                    DockableContainerFactory
                        .getFactory()
                        .createDockableContainer(replacer, DockableContainerFactory.ParentType.PARENT_SPLIT_CONTAINER);
                sdc.installDocking(this);
                if (isLeft) {
     ...
     * <p>
     * This is the method to start a nesting hierarchy inside a compound dockable (once the first dockable is added, you
     * add subsequent dockables with standard split/createTab methods)
     * <ul>
    ...
    public void addDockable(CompoundDockable base, Dockable dockable) {
        ...
        DockingActionEvent dae =
            new DockingActionAddDockableEvent(this, dockable, currentLocation, newState.getLocation(), panel);
     ...
     *
     * <p>
     * As of version 2.0 of the framework, this method can also be replaced by adding a propertychangeListener on the
     * DockKey object (and react to its DockableState property).
     *
     */
     ...
     * <p>
     * DockableStateWillChange Events are triggered <b>before</b> the state change, and are vetoable.
     */
     ...
     * <p>
     * DockingAction Events are triggered before and after any docking action (split, tab, close, hide...) and allow for
     * precise tracking (and vetoing) of these operations.
     *
     ...
     * <P>
     * this method should be called at startup time (when the dockable isn't yet displayed). to toggle a dockable from
     * docked to auto-hide, please use setAutoHide(Dockable, boolean) instead.
     *
     * <P>
     * However, this method will not fail if the dockable is already displayed : in that case, it will delegate autohide
     * to setAutoHide(Dockable, boolean), thus dropping the dockedPosition argument.
     *
     * @param dockable
     *            the dockable to add (to select the border where the dockable will be positionned, use the
     *            DockKey.setAutoHideBorder mehod
     *
     * @param dockedPosition
     *            relative positionning (may be null) indicating where to dock the dockable when leaving its auto-hide
     *            border.
     */
     ...
     *
     * <p>
     * Note that this method works by modifying the divider location of the nearest SplitContainer ancestor of the
     * dockable (the first splitted horizontally).
     *
     * <p>
     * As this method must be called when the desktop is visible, a simple way to do this is to add a window listener to
     * the parent frame of the desktop, and call this method from the windowOpened(WindowEvent) notification.
     *
     * @param width
     *            new width (if between 0 and 1, width is taken as a proportional width, otherwise it is a pixel width.
     *
     */
     ...
     * <P>
     * Note that this method works by modifying the divider location of the nearest SplitContainer ancestor of the
     * dockable (the first splitted vertically).
     *
     * <P>
     * As this method must be called when the desktop is visible, a simple way to do this is to add a window listener to
     * the parent frame of the desktop, and call this method from the windowOpened(WindowEvent) notification.
     *
     * @param height
     *            new height (if between 0 and 1, height is taken as a proportional height, otherwise it is a pixel
     *            height.
     *
     */
     ...
     * <p>
     * It is used to have a memory of grouped (tabbed) dockables in order to keep the group together when dockable are
     * restored from auto-hide mode.
     * <p>
     * This method is generally called by the tabbed container management, and not directly by the developper.
     *
     * <p>
     * However, there is a case where calling this method can be usefull : when, at startup, a desktop is built with
     * multiple hidden dockables, and the developper wants them to be grouped in a tab container when they are restored
     * to the desktop.
     *
     * <p>
     ...
     * <p>
     * For an application developper, there should be no need to call this method as it is managed internally by the
     * framework, unless the developper wants to explicitely remove a component from a tab group when this component is
     * in the auto-hide state.
     *
     * @since 1.1.2
     */
    public void removeFromTabbedGroup(Dockable dockable) { // 2005/07/13
        ...
        /*
         * This method is invoked when a component is dragged outside of a tab group : meaning the user doesn't want
         * anymore this dockable to be grouped in that tab.
         */
     ...
     * <p>
     * Warning : this method should only be used by the framework itself : changing a context "live" can have
     * unpredicted and undesired side effect.
     *
     * @since 2.1
     */
    ...
    /**
     * combines a docking action event and vetoable state change event to accept or reject a docking action.
     */
