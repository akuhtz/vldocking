...
abstract class AbstractDragControler implements DragControler {
    ...
    /*
     * This class is not public : it relies so heavily on vldocking internals that I'm reluctant to give extenders an
     * access to it.
     */
    ...
    private boolean processMouseReleased(MouseEvent e) {
        ...
        Rectangle deskBounds =
            new Rectangle(i.left, i.top, targetDesktop.getWidth() - i.left - i.right,
        ...
        if (underMouse == null) {
            // || isAncestorOf(dockableDragSource.getDockableContainer(), underMouse)){ 2007/01/08 moved to
            // implementations
    ...
    protected void processHotSwap(
        Component underMouse, Component dragged, ShapePainterStrategy shapePainterStrategy, boolean drop) { // 2005/11/08
        /*
         * This whole method should be reworked to allow hooking a DockingActionEvent subclass to process hotswapping.
         * This also means we have to get rid of these "Component" and rely on DockableContainers instead (which will
         * allow us to properly track dockable state changes)
         */
    ...
    protected UnderMouseInfo findComponentUnderMouse(MouseEvent e) {
            ...
            if (children[i] instanceof FloatingDockableContainer && children[i].isVisible()) {
                ...
                if (bounds.contains(p)) {
                    ...
                    Container contentPane =
                        (wChild instanceof JDialog) ? ((JDialog) wChild).getContentPane()
        ...
        if (umInfo.underMouse == null && desktop.getContext().getDesktopList().size() > 1) {
            ...
            if (topWindow != null) {
                ...
                Container contentPane =
                    (topWindow instanceof JDialog) ? ((JDialog) topWindow).getContentPane()
    ...
    class DragProcess {
        ...
        private boolean isSingleDockingActionAccepted(Dockable dockable) {
            ...
            // in this context, the drag event can only be generated by a hidden dockable
            // (if there is a way to drag its button) or an already docked dockable (no state change)
            if (currentState != null && currentState.getLocation() != futureLocation) { // state can be null
        ...
        private boolean checkSingleDockableWillBeDetached(Dockable dockable) {
            ...
            if (!desktop.getContext().fireDockableStateWillChange(dscwEvent)) {
                ...
                DockingActionEvent dae =
                    new DockingActionSimpleStateChangeEvent(desktop, dockable, currentState.getLocation(),
            ...
            if (checkDockableWillBeDetached()) {
                if (DockingPreferences.isLightWeightUsageEnabled() && paintFloatingDragShape) {
                    ...
                    if (dropReceiver != null) { // we are above a drop receiver and we can show something
                        ...
                        setDropShape(new Rectangle2D.Float(shapePoint.x, shapePoint.y, dragSize.width, dragSize.height),
                            shapePainterStrategy, true);
                    }
                    else {
                        setDropShape(null, shapePainterStrategy);
                    }
