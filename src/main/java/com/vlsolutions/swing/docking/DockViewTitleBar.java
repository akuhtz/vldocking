...
public class DockViewTitleBar extends JPanel implements DockableDragSource {
    ...
    public void removeNotify() {
        super.removeNotify();
        /*
         * KeyboardFocusManager.getCurrentKeyboardFocusManager().removePropertyChangeListener( focusHighlighter);
         * 2005/11/10: now it's shared between title bars
         */
        if (target != null) {
            DockKey k = target.getDockKey();
            if (k != null) {
    ...
    protected void initAutoHidePopUp(JPopupMenu popup) {
        if (DockingUtilities.isChildOfCompoundDockable(target)) {
            ...
            popup
                .add(createPopUpItem(RESTORE_TEXT, dockIcon, RESTORE_TEXT, "dock",
                    (KeyStroke) UIManager.get("DockingDesktop.dockActionAccelerator")));
        }
        if (target.getDockKey().isCloseEnabled()) {
            popup
                .add(createPopUpItem(CLOSE_TEXT, closeIcon, CLOSE_TEXT, "close",
    ...
    protected void initDockedPopUp(JPopupMenu popup) {
        ...
        if (key.isAutoHideEnabled()) {
            popup
                .add(createPopUpItem(ICONIFY_TEXT, hideIcon, ICONIFY_TEXT, "dock",
                    (KeyStroke) UIManager.get("DockingDesktop.dockActionAccelerator")));
        }
        if (key.isFloatEnabled()) {
            popup
                .add(createPopUpItem(FLOAT_TEXT, floatIcon, FLOAT_TEXT, "float",
                    (KeyStroke) UIManager.get("DockingDesktop.floatActionAccelerator")));
        }
        if (key.isMaximizeEnabled()) {
            popup
                .add(createPopUpItem(MAXIMIZE_TEXT, maximizeIcon, MAXIMIZE_TEXT, "maximize",
                    (KeyStroke) UIManager.get("DockingDesktop.maximizeActionAccelerator")));
        }
    ...
    protected void initFloatingPopUp(JPopupMenu popup) {
        if (DockingUtilities.isChildOfCompoundDockable(target)) {
            ...
            popup
                .add(createPopUpItem(ATTACH_TEXT, attachIcon, ATTACH_TEXT, "float",
                    (KeyStroke) UIManager.get("DockingDesktop.floatActionAccelerator")));
        }
    }
