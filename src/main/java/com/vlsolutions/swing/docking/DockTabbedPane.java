 ...
 * <li>layout policy is WRAP_TAB_LAYOUT (due to a java bug which limits usage of MouseMotionListeners in JTabbedPanes,
 * it is not possible to use SCROLL_LAYOUT_POLICY)
 * </ul>
 *
 * <p>
 * This tabbed pane is not meant to be used outside of DockingDesktop because of their connected behaviour (drag/drop
 * support, docking constraints, etc).
 * <p>
 * As of version 2.0 of the framework, the tabbed pane displays a close icon (other functions are available with a right
 * click menu).
 *
 * @see DockingDesktop
 *
...
public class DockTabbedPane extends JTabbedPane
    ...
    /**
     * Returns the tab index of the dockable corresponding to the given key, or null if the dockable doesn't belong to
     * this tab.
     */
    ...
    private void checkForPopUp(MouseEvent e) {
        /*
         * Tabbed dockable can be of two states : docked and floating when floating, the only option is to attach again.
         */
        Dockable d = findDockableAt(e);
        if (d != null) {
    ...
    private void scanDrop(DockEvent event, boolean drop) {
        ...
        for (int i = 0; i < getTabCount(); i++) {
            ...
            if (tabbounds != null && tabbounds.contains(p)) {
                ...
                event
                    .setDockingAction(new DockingActionCreateTabEvent(event.getDesktop(), draggedDockable,
                ...
                if (drop) {
                    if (DockingUtilities.findTabbedDockableContainer(dockable) == this) {
                        ...
                        if (base.getDockKey().getLocation() == DockableState.Location.FLOATING) { // 2005/12/09
                            if (dragSource
                                .getDockable().getDockKey().getLocation() == DockableState.Location.FLOATING) {
        ...
        Rectangle afterlast =
            new Rectangle(lasttabbounds.x + lasttabbounds.width, lasttabbounds.y,
        ...
        if (afterlast.contains(p)) {
            ...
            DockableState.Location nextLocation = base.getDockKey().getLocation();
            event
                .setDockingAction(new DockingActionCreateTabEvent(event.getDesktop(), draggedDockable, initialLocation,
                    nextLocation, base, lastTab + 1));
    ...
    private boolean scanBorderBounds(DockEvent event, boolean drop, Point p) {
        ...
        if (!innerBounds.contains(p)) {
            ...
            /*
             * NO LONGER NEEDED : REIMPLEMENTED IN BORDER SPLITTER 2005/12/08 { // we override split in order to resolve
             * the case where // the drop operation would remove this tabpane from the containment // hierarchy public
             * void split(DockDropEvent event, Container draggedContainer, DockingConstants.Split position){ if
             * (getTabCount() == 1){ // there is only one tab left, meaning // this tabbedpane has already been removed
             * from containment hierarchy Dockable remainingDockable = getDockableAt(0); event.getDesktop().split(
             * remainingDockable, // one of the remaining dockables event.getDragSource().getDockable(), position); }
             * else { super.split(event, draggedContainer, position); // standard behaviour } } }
             */ ;
            if (drop) {
